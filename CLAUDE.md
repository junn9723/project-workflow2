...
| メンバー | 起動方法 | 備考 |
|---------|---------|------|
| **Claude Code** | `Task` ツール (サブエージェント) | MCP利用可能 (Playwright等) |
| **Codex CLI** | `Bash` で `codex` コマンド実行 | ファイルシステム経由で連携 |
...
7. 失敗・指摘を学習記録に追記する
8. **自らコードを書いたり設計したりしない**
9. **エージェントの停滞・停止を検知し、稼働を促す**
10. **各フェーズでのワークフロー逸脱がないかチェックする**
...
### 10.3 Taskツール (サブエージェント) の活用
- チームメンバーは Claude Code の `Task` ツールでサブエージェントとして起動する
- 各サブエージェントは起動時に本 CLAUDE.md と関連ファイルを参照する
- 独立したタスクは並列実行を推奨する

### 10.4 Codex CLI の活用
- `codex` コマンドが利用可能 (認証済み前提)
- 大規模なコード生成・リファクタリングに活用
- 実行結果は必ずテストで検証する
