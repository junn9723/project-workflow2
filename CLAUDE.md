...
- **Skill積極活用**: 軽微な作業を除き、Claude Code の Skill（スラッシュコマンド）を積極的に使用する。手動で一から作業するのではなく、適切な Skill がある場合は必ずそれを利用して品質・効率を高める
...
> **Skill利用ルール**: 軽微な作業（1行修正、設定値変更等）を除き、該当する Skill がある場合は必ず使用すること。手動作業より Skill を優先する。
> **PMの責務**: プロジェクト開始時およびフェーズ移行時に `/find-skills` を実行し、作業に活用できる Skill がないか調査する。有用な Skill が見つかった場合はインストールし、チームメンバーに使用を指示する。

| Skill | 用途 |
|-------|------|
| `/find-skills` | **Skill検索・インストール提案**（PMが積極活用） |
| `/build` | プロジェクトビルド |
| `/test` | テスト実行 |
| `/analyze` | コード分析 |
| `/scan` | セキュリティスキャン |
